...
function DatePicker(_ref) {
  var id = _ref.id,
    name = _ref.name,
    dateSetter = _ref.dateSetter;
  var last_days = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  var months = ["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"];
  var now = new Date();
  var _React$useState = React.useState(now.getMonth()),
    _React$useState2 = _slicedToArray(_React$useState, 2),
    month = _React$useState2[0],
    setMonth = _React$useState2[1];
  var _React$useState3 = React.useState(now.getFullYear()),
    _React$useState4 = _slicedToArray(_React$useState3, 2),
    year = _React$useState4[0],
    setYear = _React$useState4[1];
  var _React$useState5 = React.useState([]),
    _React$useState6 = _slicedToArray(_React$useState5, 2),
    calendar = _React$useState6[0],
    setCalendar = _React$useState6[1];
  var _React$useState7 = React.useState(false),
    _React$useState8 = _slicedToArray(_React$useState7, 2),
    collapse = _React$useState8[0],
    setCollapse = _React$useState8[1];
  ...
  function display_month(mo, y) {
    ...
    var calend = [];
    var day = 1;
    var first_day = new Date(mo + 1 + "/" + day + "/" + y);
    if (first_day.getDay() != 1) {
      if (mo == 0) {
        var ldlm = 31;
        var ldolm = new Date("12/31/" + (y - 1));
        var day_last_month = ldolm.getDay() ? ldlm - (ldolm.getDay() - 1) : ldlm - 5;
        while (day_last_month <= ldlm) {
          calend.push(createElementFromDate(day_last_month, 11, y - 1, mo));
          day_last_month++;
        }
      } else {
        var _ldlm = is_valid_date(mo + "/" + last_days[mo - 1] + "/" + y) ? last_days[mo - 1] : last_days[mo - 1] - 1;
        var _ldolm = new Date(mo + "/" + _ldlm + "/" + y);
        var _day_last_month = _ldolm.getDay() ? _ldlm - (_ldolm.getDay() - 1) : _ldlm - 5;
        while (_day_last_month <= _ldlm) {
          calend.push(createElementFromDate(_day_last_month, mo - 1, y, mo));
          _day_last_month++;
        }
      }
    }
    var ld = is_valid_date(mo + 1 + "/" + last_days[mo] + "/" + y) ? last_days[mo] : last_days[mo] - 1;
    while (day <= ld) {
